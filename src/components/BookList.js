...
const BookList = () => {
  ...
  const fetchBooks = async () => {
    // Base endpoint
    let endpoint = `http://localhost:5000/api/books?page=${currentPage}`;
  ...
  const handleDeleteSelected = () => {
    axios
      .delete("http://localhost:5000/api/books/delete-multiple", {
  ...
  const handleDelete = async (bookId) => {
    try {
      const response = await axios.delete(
        `http://localhost:5000/api/books/destroy/${bookId}`
      );
